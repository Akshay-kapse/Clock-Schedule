...
function Schedule() {
    ...
    const fetchSchedule = async () => {
      ...
      try {
        ...
        if (Array.isArray(response.data.tasks)) {
          ...
          setCompletedTasks([]); // Ensure an empty array if tasks is not an array
        }
        setLoading(false); 
        setError(null);
      } catch (error) {
  ...
  const scheduleCreate = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4001/api/schedule/schedule",
        {
          text: newTask,
  ...
  const toggleCompleteStatus = async (taskId) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:4001/api/goal/update/${taskId}`,
  ...
  const remainingSchedule = schedule.filter((s) => !s.completed).length;
