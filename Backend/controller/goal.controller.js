...
//     // Optional: Check for duplicate goals
//     const existingGoal = await Goal.findOne({ goal, goalDate });
//     if (existingGoal) {
//       return res.status(400).json({ message: "Goal with the same title and date already exists." });
//     }
...
//   } catch (error) {
//     console.error(error);
//     res.status(500).json({ message: "Error occurred in creating goal", error: error.message });
//   }
// };
...
// {
// const { id } = req.params; // Extract the goal ID
// const { goal, goalDate } = req.body; // Extract the updated data
